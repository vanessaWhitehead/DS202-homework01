...
|          **A Good Question:**
|          A good stackoverflow question should summarize the problem at hand using correct grammar. The problem should be introduced before the code is. The code included should  be minimal and just enough should be included so that the error is reproducible. The error should also be specified
...
|          **For example:** "Why is there a *Complie Error* with some inputs but not others?"
|           I would describe the intention of the code similarly, but include a screenshot or description of the actual error.
|          Here is how I rewrote the code and a screenshot that shows how I did not run into an error. 
|          ![alt text](StackOverFlowScreenShot.png "StackOverFlow Question"){width=60%}
